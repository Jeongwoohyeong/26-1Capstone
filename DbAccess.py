    ...
    for index in range(len(data["voltage"])):
        ...
        connection.execute(
            ...
            INSERT INTO ivCurveData (
                measurementId, vMeasured, iMeasured, power, vStc, iStc
            ) VALUES (?, ?, ?, ?, ?, ?)
            """,
            (measurementId, vMeasured, iMeasured, power, vStc, iStc),
        )
...
def getCurveData(
    ...
    Returns
    -------
    list of dict : [{"vMeasured", "iMeasured", "power", "vStc", "iStc"}, ...]
    """
    cursor = connection.execute(
        """
        SELECT vMeasured, iMeasured, power, vStc, iStc
        FROM ivCurveData
        WHERE measurementId = ?
...
def getCurveDataByTime(
    ...
    Returns
    -------
    dict : {measTime: [{"vMeasured", "iMeasured", "power", "vStc", "iStc"}, ...], ...}
    """
    result = {}
    ...
    for measTime in measTimeList:
        cursor = connection.execute(
            """
            SELECT m.measTime, c.vMeasured, c.iMeasured, c.power, c.vStc, c.iStc
            FROM ivCurveData c
