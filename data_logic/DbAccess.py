...
def getFullMeasurementData(connection: sqlite3.Connection) -> List[Dict]:
    """측정 정보와 상세 데이터를 JOIN하여 전체 조회"""

    query = """
    SELECT 
        m.measurementId, m.measTime, m.channel, m.caseLevel, m.centerTemp,
        i.vMeasured, i.iMeasured, i.powerMeasured, i.vStc, i.iStc
    FROM measurementInfo m
    JOIN ivCurveData i ON m.measurementId = i.measurementId
    ORDER BY m.measTime DESC
    """
